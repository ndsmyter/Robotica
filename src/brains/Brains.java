...
public class Brains implements ListenerInterface {
    ...
    private void turn(int degrees, boolean isTurnRight) {
        ...
        move(currentState.x, currentState.y, (currentState.dir + angle + 360) % 360);
    ...
    private void drive(int distance) {
        ...
        int newx = (int) (currentState.x + distance / 10
                * Math.cos(theta) + 0.5);
        int newy = (int) (currentState.y + distance / 10
                * Math.sin(theta) + 0.5);
    ...
    private void processSensorData(){
        Sensor[] sensors = {new Sensor(10,0,0),new Sensor(10,5,0),new Sensor(10,-5,0),
            new Sensor(8,-8,-45),new Sensor(8,8,45)};
        int[] data = emulator.getSensorData();
        ArrayList<Point> obstacle = new ArrayList<Point>();
        for(int i=0; i<5; i++){
            obstacle.add(sensorDataToPoint(data[i],sensors[i]));
        }
    ...
    private Point sensorDataToPoint(int distance, Sensor sensor){
        ...
        int sensorX = (int) (currentState.x + sensor.xOffset * Math.cos(theta) + sensor.yOffset *Math.cos(theta+Math.PI/2));
        int sensorY = (int) (currentState.y + sensor.xOffset * Math.sin(theta) + sensor.yOffset *Math.sin(theta+Math.PI/2));
